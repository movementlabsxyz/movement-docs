...
export default async function Page(props: {
  ...
  // Based on available pages, we need to use ['api'] for the main api page
  const slug = params.slug ? ['api', ...params.slug] : ['api'];
  ...
  // Check if this is the main API index page
...
export async function generateStaticParams() {
  ...
  return apiSource.generateParams()
    .filter((p) => Array.isArray(p.slug) && p.slug[0] === 'api')
...
export async function generateMetadata(props: {
  ...
  return {
    ...
    openGraph: {
      title: page.data.title,
      description: page.data.description,
      url: `https://docs.movementnetwork.xyz/api${slug.length > 1 ? '/' + slug.slice(1).join('/') : ''}`,
