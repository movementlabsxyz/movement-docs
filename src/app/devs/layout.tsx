...
function rewriteUrls(node: any, base: string): any {
  ...
  if (newNode.url) {
    ...
    const filtered = parts.filter((p: string) => Boolean(p) && p !== 'devs' && p !== 'general');
...
const filterTree = (tree: any) => {
  ...
  const buildNode = tree.children.find((child: any) => child.name === 'Build');
  const filtered = {
    ...tree,
    children: buildNode ? [rewriteUrls(buildNode, '/devs')] : [],
  };
  console.log('Filtered sidebar tree for /devs:', JSON.stringify(filtered, null, 2));
  return filtered;
};
...
export default function Layout({ children }: { children: ReactNode }) {
  return (
    <div className="docs-bg relative min-h-screen">
      <DocsLayout
        tree={filterTree(devsSource.pageTree)}
        {...baseOptions}
        sidebar={{
          tabs: sidebarTabs,
        }}
