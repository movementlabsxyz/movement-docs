...
title: L1 Protocol Specification
description: "Protocol specification for Movement L1"
---

## Abstract

Movement L1 is a blockchain infrastructure built to realize the full potential of the Move programming language. It provides a secure, resource-oriented foundation for decentralized applications, on-chain finance, and digitally native organizations. 

Beyond performance and safety, Movement L1 is designed around a simple premise: technology should empower its users. It exists to restore the values of open participation, shared ownership, and verifiable trust to a global, mobile-first economy.

This document outlines the motivation, principles, and high-level architecture of Movement L1 — a people-centered Layer 1 network that unites the efficiency of Move with a design philosophy grounded in accessibility and community ownership.
...
The Move programming language, originally developed for the Diem project, represents a clean break from that trend. Its resource-oriented design, type safety, and formal verification make it uniquely suited for building verifiable financial systems. Movement L1 extends this vision to the protocol layer itself, creating a network that is both technically robust and socially aligned with the users who depend on it.

### 1.2 Vision and Motivation

Movement continues the evolution of Move-based blockchain infrastructure with a renewed focus on accessibility, community participation, and sustainable on-chain economies. 
Its aim is to refine existing foundations toward a more inclusive and purposeful network.

Modern blockchains often measure success through market metrics rather than user impact. 
Movement shifts that focus back to participation — to how effectively a network can empower people to build, earn, and coordinate. 
It is designed to transform mature technology into open infrastructure for real users.

A sovereign Layer 1 enables this direction. 
By operating its own chain, Movement can shape its economics, governance, and coordination mechanisms around inclusivity and resilience. 
The result is a network optimized for community-scale applications, real-world assets, and mobile-native finance — a system that prioritizes usability and shared benefit over speculation.
...
3. **Accessibility at the edge** — the next billion users will come from mobile, low-cost, and bandwidth-limited environments.  
...
5. **Sustainability over speculation** — value should circulate between builders, validators, and users, forming a stable foundation for long-term growth.

### 1.4 Applications and Ecosystem

Movement L1 is designed as a general-purpose Layer 1 that supports a wide spectrum of decentralized applications, from on-chain finance to digital identity and coordination systems. Its architecture emphasizes composability, verifiability, and predictable performance, allowing developers to build without compromising safety or accessibility.
...
- **Real-world asset integration** — tokenized representations of tangible assets whose ownership and yield can be distributed among users rather than concentrated in custodial entities.  
...
Movement L1 is a social-technical system. Its purpose is not to create another marketplace for tokens but to establish a durable framework for coordination and ownership. The network treats every contribution — from validation to development, from governance to education — as a building block of value. 
...
2. **High-Throughput Consensus**: [Jolteon](https://arxiv.org/abs/2106.10362) lineage and a [Quorum Store](https://forum.aptosfoundation.org/t/aptos-quorum-store-breaking-the-bottleneck/16671)–like dissemination layer for scaling under load. 
...
Movement L1's MoveVM implementation includes an _**optimized interpreter**_ with verified bytecode, _**instruction caching**_ for caching of compiled bytecode for repeated execution, _**gas optimization**_ with precise gas metering and minimal overhead, and _**memory management**_ with efficient memory allocation and garbage collection.
...
Movement L1 maintains a global state organized by account addresses using an _**account model**_ where each address can hold multiple resources and modules, _**resource storage**_ with native storage of Move resources and type information, _**module storage**_ for compiled Move modules with metadata, and _**versioned state**_ indexed by ledger version, with authenticated proofs via the Jellyfish Merkle Tree.
...
Movement L1 uses Ed25519 signatures for _**transaction authentication**_ providing proof of transaction authorization, _**validator signatures**_ for consensus vote authentication, _**message integrity**_ for P2P message authentication, and _**multi-signatures**_ via Multi-Ed25519 threshold schemes.
