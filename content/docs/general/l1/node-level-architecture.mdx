...
This document provides a detailed view of the internal architecture of M1 nodes, covering the various components, their interactions, and the data flows within a single node instance. For the high-level system architecture and network design, see [M1 Architecture](/general/l1/architecture).

## Core Layers

The M1 node architecture is organized into five core layers, each handling specific aspects of node functionality while working together to provide a complete, high-performance blockchain node. These layers correspond directly to the [M1 Architecture](/general/l1/architecture) but focus on the internal implementation details of individual nodes rather than the overall system design.
...
The RPC/API Server provides _**transaction submission**_ endpoints for submitting signed transactions to the network, _**state queries**_ for querying account states and resource data, _**block information**_ retrieval for block and transaction data, and _**network status**_ endpoints for getting node and network health information. For real-time data access, applications must implement polling mechanisms using these RPC endpoints.

#### WebSocket Server

Nodes do not provide WebSocket endpoints for real-time block or event streaming at this point. The WebSocket Server component is not available for external clients. For real-time data access, applications must use polling via RPC endpoints or utilize third-party indexer services.
