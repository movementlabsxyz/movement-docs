...
## Option 1: Building Docker Images from Source

If you want to build your own Docker images from the latest source code or customize the build, you can use the Movement [aptos-core](https://github.com/movementlabsxyz/aptos-core) repository with the `just` command runner.
...
1. Clone the Movement `aptos-core` repository:
...
2. Build the Docker image for `aptos-node`:

```bash filename="Terminal"
# Build with default (dev) profile
just docker-build

# Build with release profile for production use
just docker-build aptos-node latest release

# Build with custom tag
just docker-build aptos-node my-custom-tag release
```

3. The build process will:
   - Automatically build the required `aptos-node` binary using Nix
   - Create a Docker image with the specified tag
   - Handle complex dependencies like RocksDB and OpenSSL automatically

4. Once built, you can use your custom image in place of the official `aptoslabs/validator:mainnet` image in the docker run command below.

<Callout type="info">
**Build Profiles**<br />
- `dev` (default) - Development build with debug symbols
- `release` - Optimized release build recommended for production
- The Nix build system ensures reproducible builds across different environments
</Callout>

<Callout type="info">
**Troubleshooting Docker Builds**<br />
...
4. List available commands: `just help`
</Callout>
