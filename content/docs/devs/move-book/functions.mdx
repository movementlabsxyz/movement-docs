...
The `entry` modifier is designed to allow module functions to be safely and directly invoked much like scripts. This allows module writers to specify which functions can be used to begin execution. The module writer then knows that any non-entry function will be called from a Move program already in execution.
...
3. **Use appropriate visibility** - Only make functions `public` when necessary
4. **Document acquires** - Always specify `acquires` annotations for global storage access
5. **Handle errors gracefully** - Use `assert!` or return `Option` types for error handling
6. **Leverage type parameters** - Use generics to make functions more reusable
...
module 0x42::counter {
    ...
    public fun add(addr: address, amount: u64) acquires Counter {
        let counter = borrow_global_mut<Counter>(addr);
...
|------------|---------------|----------|
| (default) | Same module only | Internal helper functions |
| `public` | Any module or script | Public APIs |
| `public(friend)` | Same module or friend modules | Controlled access |
| `entry` | Can be transaction entry point | Main functions |

Understanding functions is fundamental to Move programming, as they are the primary way to organize and reuse code while maintaining Move's safety guarantees.