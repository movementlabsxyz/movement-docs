...
For more details, see the type abilities section.
...
### Explicit Copying for Complex Types

However, this is not permitted for fields that contain non-primitive types, such as a vector or another struct:
...
In addition to reading from fields, the dot syntax can be used to modify fields, regardless of the field being a primitive type or some other struct.
...
Following these rules, if you want to modify your struct outside the module, you will need to provide public APIs for them.
...
1. **Choose appropriate abilities** - Only grant abilities that are necessary for your use case
2. **Use resources for valuable data** - Don't give `copy` or `drop` to structs representing assets
3. **Provide public APIs** - Create accessor and mutator functions for external module interaction
4. **Use descriptive names** - Follow the capitalization rules and use meaningful struct names
5. **Consider ownership patterns** - Design your APIs to make ownership transfer clear
6. **Leverage pattern matching** - Use destructuring for clean resource handling
...
module 0x2::bank {
    ...
    public fun deposit(account: address, amount: u64) acquires Account {
        let account_ref = borrow_global_mut<Account>(account);
...
module 0x2::config {
    ...
    public fun set_max_users(config: &mut Config, max_users: u64) {
        config.max_users = max_users;
    }

    public fun set_timeout(config: &mut Config, timeout: u64) {
