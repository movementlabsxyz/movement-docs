...
The [`@movement-network/near-intents-sdk`](https://github.com/MoveIndustries/near-intents-sdk) is a thin TypeScript SDK for moving **USDC or USDT** from a supported origin chain onto **Movement** — landing as **USDCx** or **MOVE** — over [NEAR Intents](https://docs.near-intents.org). It wraps the official [1Click API](https://docs.near-intents.org/near-intents/integration/distribution-channels/1click-api) and pins the destination to Movement, so a transfer can't accidentally land on the wrong chain.
...
```bash tab="npm"
npm install @movement-network/near-intents-sdk
```

```bash tab="pnpm"
pnpm add @movement-network/near-intents-sdk
```

```bash tab="yarn"
yarn add @movement-network/near-intents-sdk
```

```bash tab="bun"
bun add @movement-network/near-intents-sdk
```
...
import {
  ...
  getStatus,
  isTerminal,
} from "@movement-network/near-intents-sdk";
...
```ts
import { getStatus, isTerminal } from "@movement-network/near-intents-sdk";
...
import { configure, quoteDeposit, prepareDepositTx, submitDeposit } from "@movement-network/near-intents-sdk";
...
```ts
import { listTokens } from "@movement-network/near-intents-sdk";
