...
Running a full node locally allows you to evaluate performance on a given network. If you are joining a high-load network, such as Movement Testnet, we recommend the following specs:
...
The current container revision for installation is: `CONTAINER_REV=07200fc-amd64`, GitHub commit: `07200fc880954dc85d0760480affa09299074107`

## Running a Movement Full Node

You can join any network as a full node by running a Movement node using container tags that point to the latest commit hash, along with the [`fullnode`](https://github.com/movementlabsxyz/movement/blob/feature/da-sequencer-node/docker/compose/movement-full-node/docker-compose.fullnode.yml) overlay.
...
ansible-playbook --inventory <your-inventory> \
    ...
    --extra-vars "movement_repo_commit=${MOVEMENT_COMMIT}" \
    --extra-vars "user=ubuntu" \
    docs/movement-node/run-fullnode/ansible/mainnet/movement-node.yml \
    --private-key your-private-key.pem
```
...
If you are creating a new node add these environment variables:
...
If values do not converge after some time, or the difference remains constant, the full node is likely out of sync. To solve this, try restoring it again. If the issue persists, contact the Movement team to help troubleshoot.
...
The full node batch signer key is not whitelisted. Send it to the Movement team so that it can be registered to the da-sequencer.

### The node is blocked and doesn't start

If the node starts and then stops with the following logs:
...
It means that the `config.json` file hasn't been properly configured. Run the `setup_migration` script (see the `Update Config` section above).

### Error "Send execution state to da sequencer failed"

Update config.json to set the `propagate_execution_state` field to `false` and restart.

### Error "The node shouldn't send state to the DA, change the config propagate_execution_state to "propagate_execution_state": false and restart"

The `maptos` database is not the one expected by the node. Stop the node and perform a restoration.

### Error "WARN movement_da_sequencer_client: DA sequencer Http2 connection url:... failed"

The da-sequencer connection URL is incorrectly configured. Edit the `config.json` field `connection_url` and set it to the value corresponding the connected network defined in the `Update Config` section.

### Error "movement-restore-db  | unknown command "latest" for "restic"

The environment is needed by the Docker script, or it is not properly defined or detected by Docker.
...
Sensitive data and some unused data has been removed:
