...
```
npx --legacy-peer-deps create-react-app client --template typescript
```

Why --legacy-peers-deps is needed: https://github.com/facebook/react/issues/31699
...
Create constants.ts file in the client's src directory with contents:

```
export const ONCHAIN_BIO = "<your-account-address>";
```

Install these required packages and run the application again:

```
npm --legacy-peer-deps i petra-plugin-wallet-adapter aptos @aptos-labs/ts-sdk @aptos-labs/wallet-adapter-ant-design @aptos-labs/wallet-adapter-react @telegram-apps/bridge @mizuwallet-sdk/core graphql-request graphql
npm start
```

Update versions of Node.js and npm, if needed: https://nodejs.org/en/download/
...
Full example available at: https://github.com/movementlabsxyz/onchain-bio-dapp 
